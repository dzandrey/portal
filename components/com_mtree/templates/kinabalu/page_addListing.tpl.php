...
<form action="<?php echo JRoute::_("index.php") ?>" method="post" enctype="multipart/form-data" name="mtForm" id="mtForm">
	...
	<li>
		<input type="button" value="<?php echo JText::_( 'COM_MTREE_SUBMIT_LISTING' ) ?>" onclick="javascript:submitbutton('savelisting')" class="button" /> <input type="button" value="<?php echo JText::_( 'COM_MTREE_CANCEL' ) ?>" onclick="history.back();" class="button" />
	</li>
	<li class="required-note"><?php echo JText::_('COM_MTREE_LISTING_SUBMIT_ASTERISK_REQUIRED'); ?></li>
	<li>
		<span class="mtcaption">
	...
	while( $this->fields->hasNext() ) {
		...
		if($field->hasInputField()) {
			...
			if($field->getCaption() != false) {
				echo '<span class="mtcaption" id="caption_'.$field->getId().'">';
			...
			echo '<span class="mtinput" id="input_'.$field->getId().'">';
